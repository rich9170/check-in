...
export function TherapistCard({ therapist, onSelect, index }) {
  ...
  return (
    <button
      ...
      className="group animate-fade-up flex min-h-[200px] flex-col overflow-hidden rounded-lg border-[1.5px] border-brand-line bg-white text-left shadow-[0_4px_18px_rgba(34,48,42,0.06)] transition-[transform,box-shadow,border-color] duration-200 ease-out hover:-translate-y-1 hover:border-brand-green hover:shadow-[0_14px_34px_rgba(46,93,58,0.18)] active:translate-y-0 active:scale-[0.985] focus:outline-none focus-visible:ring-4 focus-visible:ring-brand-green/40"
    >
      <div className="aspect-square w-full overflow-hidden bg-brand-cream-deep">
        <img
          src={photo}
          alt={name}
          draggable={false}
      ...
      <div className="flex flex-1 flex-col justify-center px-5 py-4">
        <span className="font-serif text-[1.5rem] leading-tight text-brand-ink">
          {name}
        </span>
        {credentials ? (
          <span className="mt-1 text-sm font-medium uppercase tracking-wide text-brand-green">
            {credentials}
          </span>
        ) : null}
      </div>
